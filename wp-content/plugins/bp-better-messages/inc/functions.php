    ...
    class BP_Better_Messages_Functions
    {
        ...
        public function delete_message( $message_id, $thread_id = false ){
            ...
            if( $thread_id !== false ) {
                ...
                BP_Better_Messages()->functions->decrease_unread( $thread_id, array_keys( $recipients ) );
            }
        ...
        public function find_existing_threads($from, $to){
            ...
            $query_from = $wpdb->prepare("SELECT
                ...
                AND recipients.is_deleted = 0
                AND recipients.thread_id NOT IN ({$threads_excluded})
                ", $from);
        ...
        function delete_all_thread_meta( $thread_id ) {
            global $wpdb;
            $table = bpbm_get_table('threadsmeta');

            return $wpdb->query($wpdb->prepare("DELETE FROM {$table} WHERE `bpbm_threads_id` = %d", $thread_id) );
        }
        ...
        public function new_message( $args = '' ) {
            ...
            if( $new_thread || $r['new_thread'] === true ){
                BP_Better_Messages()->functions->delete_all_thread_meta( $message->thread_id );
