...
function video_conference_zoom_meeting_end_author() {
	...
	wp_localize_script( 'speed-dating-with-zoom', 'vczapi_state', $data );
	?>
    <div class="dpn-zvc-sidebar-state">
		<?php if ( empty( $meeting->state ) ) { ?>
            <a href="javascript:void(0);" class="vczapi-meeting-state-change" data-type="post_type" data-state="end" data-postid="<?php echo $post->ID; ?>" data-id="<?php echo $meeting->id ?>"><?php _e( 'Приключи срещата ?', 'speed-dating-with-zoom' ); ?></a>
		<?php } else { ?>
            <a href="javascript:void(0);" class="vczapi-meeting-state-change" data-type="post_type" data-state="resume" data-postid="<?php echo $post->ID; ?>" data-id="<?php echo $meeting->id ?>"><?php _e( 'Позволи присъединяване в срещата?', 'speed-dating-with-zoom' ); ?></a>
		<?php } ?>
        <p><?php _e( 'You are seeing this because you are the author of this post.', 'speed-dating-with-zoom' ); ?></p>
    </div>
	<?php
}
...
function video_conference_zoom_meeting_join() {
	...
	if ( empty( $zoom['api']->state ) && video_conference_zoom_check_login() ) {
		...
		wp_localize_script( 'speed-dating-with-zoom', 'mtg_data', $data );
	} else {
		echo "<p>" . __( 'Please login to join this meeting.', 'speed-dating-with-zoom' ) . "</p>";
	}
}
...
function video_conference_zoom_meeting_join_link( $zoom_meeting ) {
	...
	if ( ! empty( $zoom_meeting->join_url ) && ! $disable_app_join ) {
		...
		?>
        <a target="_blank" href="<?php echo esc_url( $join_url ); ?>" class="btn btn-join-link btn-join-via-app"><?php echo apply_filters( 'vczapi_join_meeting_via_app_text', __( 'Влез чрез Zoom', 'speed-dating-with-zoom' ) ); ?></a>
		<?php
	}
...
function video_conference_zoom_shortcode_join_link_webinar( $zoom_webinars ) {
	if ( empty( $zoom_webinars ) ) {
		echo "<p>" . __( 'Webinar is not defined. Try updating this Webinar', 'speed-dating-with-zoom' ) . "</p>";
...
function video_conference_zoom_shortcode_join_link( $zoom_meetings ) {
	if ( empty( $zoom_meetings ) ) {
		echo "<p>" . __( 'Meeting is not defined. Try updating this meeting', 'speed-dating-with-zoom' ) . "</p>";
	...
	function video_conference_zoom_shortcode_table( $zoom_meetings ) {
        ...
        <table class="vczapi-shortcode-meeting-table">
            <tr class="vczapi-shortcode-meeting-table--row1">
                <td><?php _e( 'Meeting ID', 'speed-dating-with-zoom' ); ?></td>
                <td><?php echo $zoom_meetings->id; ?></td>
            </tr>
            <tr class="vczapi-shortcode-meeting-table--row2">
                <td><?php _e( 'Topic', 'speed-dating-with-zoom' ); ?></td>
                <td><?php echo $zoom_meetings->topic; ?></td>
            </tr>
            <tr class="vczapi-shortcode-meeting-table--row3">
                <td><?php _e( 'Meeting Status', 'speed-dating-with-zoom' ); ?></td>
                <td>
					<?php
					if ( $zoom_meetings->status === "waiting" ) {
						_e( 'Waiting - Not started', 'speed-dating-with-zoom' );
					} else if ( $zoom_meetings->status === "started" ) {
						_e( 'Meeting is in Progress', 'speed-dating-with-zoom' );
					} else {
						echo $zoom_meetings->status;
					}
					?>
                    <p class="small-description"><?php _e( 'Refresh is needed to change status.', 'speed-dating-with-zoom' ); ?></p>
                </td>
            </tr>
			<?php
			if ( ! empty( $zoom_meetings->type ) && $zoom_meetings->type === 8 ) {
				if ( ! empty( $zoom_meetings->occurrences ) ) {
                    ...
                    <tr class="vczapi-shortcode-meeting-table--row4">
                        <td><?php _e( 'Type', 'speed-dating-with-zoom' ); ?></td>
                        <td><?php _e( 'Recurring Meeting', 'speed-dating-with-zoom' ); ?></td>
                    </tr>
                    <tr class="vczapi-shortcode-meeting-table--row4">
                        <td><?php _e( 'Ocurrences', 'speed-dating-with-zoom' ); ?></td>
                        <td><?php echo count( $zoom_meetings->occurrences ); ?></td>
                    </tr>
                    <tr class="vczapi-shortcode-meeting-table--row5">
                        <td><?php _e( 'Next Start Time', 'speed-dating-with-zoom' ); ?></td>
                        <td>
							<?php
							$now               = new DateTime( 'now -1 hour', new DateTimeZone( $zoom_meetings->timezone ) );
							$closest_occurence = false;
							if ( ! empty( $zoom_meetings->type ) && $zoom_meetings->type === 8 && ! empty( $zoom_meetings->occurrences ) ) {
								foreach ( $zoom_meetings->occurrences as $occurrence ) {
									if ( $occurrence->status === "available" ) {
										...
										_e( 'Meeting has ended !', 'speed-dating-with-zoom' );
										break;
									}
								}
							}
                    ...
                    <tr class="vczapi-shortcode-meeting-table--row6">
                        <td><?php _e( 'Start Time', 'speed-dating-with-zoom' ); ?></td>
                        <td><?php _e( 'Meeting has ended !', 'speed-dating-with-zoom' ); ?></td>
                    </tr>
					<?php
				}
			} else if ( ! empty( $zoom_meetings->type ) && $zoom_meetings->type === 3 ) {
                ...
                <tr class="vczapi-shortcode-meeting-table--row6">
                    <td><?php _e( 'Start Time', 'speed-dating-with-zoom' ); ?></td>
                    <td><?php _e( 'This is a meeting with no Fixed Time.', 'speed-dating-with-zoom' ); ?></td>
                </tr>
				<?php
			} else if ( ! empty( $zoom_meetings->type ) && $zoom_meetings->type === 4 ) {
				?>
                <tr class="vczapi-shortcode-meeting-table--row6">
                    <td><?php _e( 'Type', 'speed-dating-with-zoom' ); ?></td>
                    <td><?php _e( 'Personal Meeting Room', 'speed-dating-with-zoom' ); ?></td>
                </tr>
				<?php
...
function video_conference_zoom_after_jbh_html() {
	...
	?>
    <script id='speed-dating-with-zoom-browser-js-extra'>
