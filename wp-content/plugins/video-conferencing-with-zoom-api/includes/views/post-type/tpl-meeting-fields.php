...
<table class="form-table">
	...
	if ( $post->post_status == 'publish' && is_object( $meeting_details ) && isset( $meeting_details->id ) ) {
        ...
        <tr>
            ...
            <td>
				...
				<?php } ?>
                <p class="description">
					<?php _e( 'If you need to show this meeting on another page or post please use this shortcode', 'speed-dating-with-zoom' ); ?>
                </p>
            </td>
        </tr>
		<?php
	...
	if ( ! empty( $meeting_details ) && ! empty( $meeting_details->id ) && ( $post->post_status === 'publish' || $post->post_status === 'draft' || $post->post_status === 'pending' ) ) { ?>
        <tr class="zoom-host-id-selection-admin">
            <th scope="row"><label for="userId"><?php _e( 'Meeting Host *', 'speed-dating-with-zoom' ); ?></label></th>
            <td>
				<?php
				if ( ! empty( $meeting_details->host_id ) ) {
					...
					if ( ! empty( $user ) ) {
						...
						_e( 'Please check your internet connection or API connection.', 'speed-dating-with-zoom' );
					}
				} else {
					printf( __( 'Did not find any hosts here ? Please %scheck here%s to verify your API keys are working correctly.', 'speed-dating-with-zoom' ), '<a href="' . admin_url( 'edit.php?post_type=zoom-meetings&page=zoom-video-conferencing-settings' ) . '">', '</a>' );
				} ?>
                <p class="description" id="userId-description"><?php _e( 'This is host ID for the meeting (Required).', 'speed-dating-with-zoom' ); ?></p>
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="meeting_type"><?php _e( 'Meeting Type', 'speed-dating-with-zoom' ); ?></label></th>
            <td>
                <p><?php echo ! empty( $meeting_fields['meeting_type'] ) && $meeting_fields['meeting_type'] === 2 ? __( 'Zoom Webinar', 'speed-dating-with-zoom' ) : __( 'Zoom Meeting', 'speed-dating-with-zoom' ); ?></p>
                <p class="description"><?php _e( 'You cannot update meeting type. This is not allowed to avoid any conflict issues.', 'speed-dating-with-zoom' ); ?></p>
                ...
                <td>
					...
					if ( ! empty( $users ) ) {
						...
						<?php } else { ?>
                            ...
                            <p class="vczapi-manually-hostid-wrap"><a href="javascript:void(0);" class="vczapi-admin-hostID-manually-add"><?php _e( 'User not in the list? Click here to manually enter Host.', 'speed-dating-with-zoom' ); ?></a></p>
						<?php } ?>
                        <p class="description" id="userId-description"><?php _e( 'This is host ID for the meeting (Required).', 'speed-dating-with-zoom' ); ?></p>
					<?php } else {
						printf( __( 'Did not find any hosts here ? Please %scheck here%s to verify your API keys are working correctly.', 'speed-dating-with-zoom' ), '<a href="' . admin_url( 'edit.php?post_type=zoom-meetings&page=zoom-video-conferencing-settings' ) . '">', '</a>' );
        ...
        <tr class="zoom-meeting-type-selection-admin">
            ...
            <td>
                ...
                <p class="description" id="userId-description"><?php _e( 'Which type of meeting do you want to create. Note: Webinar requires Zoom Webinar Plan enabled in your account.', 'speed-dating-with-zoom' ); ?>
                    ?</p>
            </td>
        </tr>
		<?php
    ...
    <tr>
        ...
        <td>
            ...
            <p class="description" id="start_date-description"><?php _e( 'Starting Date and Time of the Meeting (Required).', 'speed-dating-with-zoom' ); ?></p>
        </td>
    </tr>
    ...
    <tr>
        ...
        <td>
            ...
            <p class="description" id="timezone-description"><?php _e( 'Meeting Timezone', 'speed-dating-with-zoom' ); ?></p>
        </td>
    </tr>
    <tr>
    ...
    <tr>
        <th scope="row"><label for="password"><?php _e( 'Password', 'speed-dating-with-zoom' ); ?></label></th>
    ...
    <tr>
        <th scope="row">
            <label for="disable-waiting-room"><?php _e( 'Disable Waiting Room', 'speed-dating-with-zoom' ); ?></label></th>
        <td>
            <p class="description" id="disable-waiting-room">
                ...
                <span style="color:red"><?php _e( 'Please keep in mind anyone with the meeting link will be able to join without you allowing them into the meeting.', 'speed-dating-with-zoom' ) ?></span>
            </p>
        </td>
    </tr>
    <tr>
        <th scope="row"><label for="meeting-authentication"><?php _e( 'Meeting Authentication', 'speed-dating-with-zoom' ); ?></label></th>
        <td>
            <p class="description" id="meeting-authentication">
                <input type="checkbox" name="meeting_authentication" value="1" <?php ! empty( $meeting_fields['meeting_authentication'] ) ? checked( '1', $meeting_fields['meeting_authentication'] ) : false; ?> class="regular-text"><?php _e( 'Only loggedin users in Zoom App can join this Meeting.', 'speed-dating-with-zoom' ); ?>
            </p>
        </td>
    </tr>
    <tr class="vczapi-admin-hide-on-webinar" <?php echo ! empty( $meeting_fields['meeting_type'] ) && $meeting_fields['meeting_type'] === 2 ? 'style="display: none;"' : false; ?>>
        <th scope="row"><label for="join_before_host"><?php _e( 'Join Before Host', 'speed-dating-with-zoom' ); ?></label></th>
        <td>
            <p class="description" id="join_before_host-description">
                <input type="checkbox" name="join_before_host" value="1" <?php ! empty( $meeting_fields['join_before_host'] ) ? checked( '1', $meeting_fields['join_before_host'] ) : false; ?> class="regular-text"><?php _e( 'Allow users to join meetin before host start/joins the meeting. Only for scheduled or recurring meetings. If the waiting room is enabled, this setting will not work.', 'speed-dating-with-zoom' ); ?>
            </p>
        </td>
    </tr>
    <tr>
        <th scope="row"><label for="option_host_video"><?php _e( 'Start When Host Joins', 'speed-dating-with-zoom' ); ?></label></th>
        <td>
            <p class="description" id="option_host_video-description">
                <input type="checkbox" name="option_host_video" value="1" <?php ! empty( $meeting_fields['option_host_video'] ) ? checked( '1', $meeting_fields['option_host_video'] ) : false; ?> class="regular-text"><?php _e( 'Start video when host join meeting.', 'speed-dating-with-zoom' ); ?>
            </p>
        </td>
    </tr>
    <tr class="vczapi-admin-hide-on-webinar" <?php echo ! empty( $meeting_fields['meeting_type'] ) && $meeting_fields['meeting_type'] === 2 ? 'style="display: none;"' : false; ?>>
        <th scope="row"><label for="option_participants_video"><?php _e( 'Participants Video', 'speed-dating-with-zoom' ); ?></label></th>
        <td>
            <p class="description" id="option_participants_video-description">
                <input type="checkbox" name="option_participants_video" <?php ! empty( $meeting_fields['option_participants_video'] ) ? checked( '1', $meeting_fields['option_participants_video'] ) : false; ?> value="1" class="regular-text"><?php _e( 'Start video when participants join meeting.', 'speed-dating-with-zoom' ); ?>
            </p>
        </td>
    </tr>
    <tr class="vczapi-admin-hide-on-webinar" <?php echo ! empty( $meeting_fields['meeting_type'] ) && $meeting_fields['meeting_type'] === 2 ? 'style="display: none;"' : false; ?>>
        <th scope="row">
            <label for="option_mute_participants_upon_entry"><?php _e( 'Mute Participants upon entry', 'speed-dating-with-zoom' ); ?></label>
        </th>
        <td>
            <p class="description" id="option_mute_participants_upon_entry">
                <input type="checkbox" name="option_mute_participants" value="1" <?php ! empty( $meeting_fields['option_mute_participants'] ) ? checked( '1', $meeting_fields['option_mute_participants'] ) : false; ?> class="regular-text"><?php _e( 'Mutes Participants when entering the meeting.', 'speed-dating-with-zoom' ); ?>
            </p>
        </td>
    </tr>
    <tr class="vczapi-admin-show-on-webinar" <?php echo ! empty( $meeting_fields['meeting_type'] ) && $meeting_fields['meeting_type'] === 1 ? 'style="display: none;"' : false; ?>>
        <th scope="row">
            <label for="panelists_video"><?php _e( 'When Panelists Join', 'speed-dating-with-zoom' ); ?></label>
        </th>
        <td>
            <p class="description">
                <input type="checkbox" name="panelists_video" value="1" <?php ! empty( $meeting_fields['panelists_video'] ) ? checked( '1', $meeting_fields['panelists_video'] ) : false; ?> class="regular-text"><?php _e( 'Start video when panelists join webinar.', 'speed-dating-with-zoom' ); ?>
            </p>
        </td>
    </tr>
    <tr class="vczapi-admin-show-on-webinar" <?php echo ! empty( $meeting_fields['meeting_type'] ) && $meeting_fields['meeting_type'] === 1 ? 'style="display: none;"' : false; ?>>
        <th scope="row">
            <label for="practice_session"><?php _e( 'Practise Session', 'speed-dating-with-zoom' ); ?></label>
        </th>
        <td>
            <p class="description">
                <input type="checkbox" name="practice_session" value="1" <?php ! empty( $meeting_fields['practice_session'] ) ? checked( '1', $meeting_fields['practice_session'] ) : false; ?> class="regular-text"><?php _e( 'Enable Practise Session.', 'speed-dating-with-zoom' ); ?>
            </p>
        </td>
    </tr>
    <tr class="vczapi-admin-show-on-webinar" <?php echo ! empty( $meeting_fields['meeting_type'] ) && $meeting_fields['meeting_type'] === 1 ? 'style="display: none;"' : false; ?>>
        <th scope="row">
            <label for="hd_video"><?php _e( 'HD Video', 'speed-dating-with-zoom' ); ?></label>
        </th>
        <td>
            <p class="description">
                <input type="checkbox" name="hd_video" value="1" <?php ! empty( $meeting_fields['hd_video'] ) ? checked( '1', $meeting_fields['hd_video'] ) : false; ?> class="regular-text"><?php _e( 'Defaults to HD video.', 'speed-dating-with-zoom' ); ?>
            </p>
        </td>
    </tr>
    <tr class="vczapi-admin-show-on-webinar" <?php echo ! empty( $meeting_fields['meeting_type'] ) && $meeting_fields['meeting_type'] === 1 ? 'style="display: none;"' : false; ?>>
        <th scope="row">
            <label for="allow_multiple_devices"><?php _e( 'Allow Multiple Devices', 'speed-dating-with-zoom' ); ?></label>
        </th>
        <td>
            <p class="description">
                <input type="checkbox" name="allow_multiple_devices" value="1" <?php ! empty( $meeting_fields['allow_multiple_devices'] ) ? checked( '1', $meeting_fields['allow_multiple_devices'] ) : false; ?> class="regular-text"><?php _e( 'Allow attendess to join from multiple devices.', 'speed-dating-with-zoom' ); ?>
            </p>
        </td>
    </tr>
    <tr>
        ...
        <td>
            <select id="option_auto_recording" name="option_auto_recording">
                <option value="none" <?php ! empty( $meeting_fields['option_auto_recording'] ) ? selected( 'none', $meeting_fields['option_auto_recording'] ) : false; ?>>
					<?php _e( 'No Recordings', 'speed-dating-with-zoom' ); ?>
                </option>
                <option value="local" <?php ! empty( $meeting_fields['option_auto_recording'] ) ? selected( 'local', $meeting_fields['option_auto_recording'] ) : false; ?>>
					<?php _e( 'Local', 'speed-dating-with-zoom' ); ?>
                </option>
                <option value="cloud" <?php ! empty( $meeting_fields['option_auto_recording'] ) ? selected( 'cloud', $meeting_fields['option_auto_recording'] ) : false; ?>>
					<?php _e( 'Cloud', 'speed-dating-with-zoom' ); ?>
                </option>
            </select>
	...
	if ( $show_host ) {
		?>
        <tr>
            <th scope="row"><label for="settings_alternative_hosts"><?php _e( 'Alternative Hosts', 'speed-dating-with-zoom' ); ?></label>
            </th>
            <td>
				<?php if ( ! empty( $users ) ) { ?>
                ...
                <p class="description" id="settings_alternative_hosts"><?php _e( 'Paid Zoom Account is required for this !! Alternative hosts IDs. Multiple value separated by comma.', 'speed-dating-with-zoom' ); ?></p>
            </td>
        </tr>
	<?php } ?>
    </tbody>
