...
final class Bootstrap {
	...
	function enqueue_scripts() {
		...
		wp_register_style( 'speed-dating-with-zoom', ZVC_PLUGIN_PUBLIC_ASSETS_URL . '/css/style' . $minified . '.css', false, $this->plugin_version );
		//Enqueue MomentJS
		wp_register_script( 'speed-dating-with-zoom-moment', ZVC_PLUGIN_VENDOR_ASSETS_URL . '/moment/moment.min.js', array( 'jquery' ), $this->plugin_version, true );
		wp_register_script( 'speed-dating-with-zoom-moment-locales', ZVC_PLUGIN_VENDOR_ASSETS_URL . '/moment/moment-with-locales.min.js', array(
			'jquery',
			'speed-dating-with-zoom-moment'
		), $this->plugin_version, true );
		//Enqueue MomentJS Timezone
		wp_register_script( 'speed-dating-with-zoom-moment-timezone', ZVC_PLUGIN_VENDOR_ASSETS_URL . '/moment-timezone/moment-timezone-with-data-10-year-range.min.js', array( 'jquery' ), $this->plugin_version, true );
		wp_register_script( 'speed-dating-with-zoom', ZVC_PLUGIN_PUBLIC_ASSETS_URL . '/js/public' . $minified . '.js', array(
			'jquery',
			'speed-dating-with-zoom-moment'
		), $this->plugin_version, true );
		if ( is_singular( 'zoom-meetings' ) ) {
			...
			$translation_array   = apply_filters( 'vczapi_meeting_event_text', array(
				'meeting_started'  => ! empty( $zoom_started ) ? $zoom_started : __( 'Meeting Has Started ! Click below join button to join meeting now !', 'speed-dating-with-zoom' ),
				'meeting_starting' => ! empty( $zoom_going_to_start ) ? $zoom_going_to_start : __( 'Click join button below to join the meeting now !', 'speed-dating-with-zoom' ),
				'meeting_ended'    => ! empty( $zoom_ended ) ? $zoom_ended : __( 'This meeting has been ended by the host.', 'speed-dating-with-zoom' ),
				'date_format'      => $date_format
			) );
			wp_localize_script( 'speed-dating-with-zoom', 'zvc_strings', $translation_array );
		}
	...
	public function enqueue_scripts_backend( $hook ) {
		...
		if ( $hook === $pg . "video-conferencing-addons" || $hook === $pg . "video-conferencing-reports" || $hook === $pg . "video-conferencing-recordings" || $hook === $pg . "video-conferencing-list-users" || $hook === $pg . "video-conferencing" || $hook === $pg . "video-conferencing-add-meeting" || $hook === $pg . "video-conferencing-webinars" || $hook === $pg . "video-conferencing-webinars-add" || $screen->id === "zoom-meetings" || $hook === $pg . "video-conferencing-host-id-assign" || $hook === $pg . "video-conferencing-sync" || $hook === $pg . "video-conferencing-add-users" ) {
			wp_enqueue_style( 'speed-dating-with-zoom-timepicker', ZVC_PLUGIN_VENDOR_ASSETS_URL . '/dtimepicker/jquery.datetimepicker.min.css', false, $this->plugin_version );
			wp_enqueue_style( 'speed-dating-with-zoom-select2', ZVC_PLUGIN_VENDOR_ASSETS_URL . '/select2/css/select2.min.css', false, $this->plugin_version );
			wp_enqueue_style( 'speed-dating-with-zoom-datable', ZVC_PLUGIN_VENDOR_ASSETS_URL . '/datatable/jquery.dataTables.min.css', false, $this->plugin_version );
		}

		wp_register_script( 'speed-dating-with-zoom-select2-js', ZVC_PLUGIN_VENDOR_ASSETS_URL . '/select2/js/select2.min.js', array( 'jquery' ), $this->plugin_version, true );
		wp_register_script( 'speed-dating-with-zoom-timepicker-js', ZVC_PLUGIN_VENDOR_ASSETS_URL . '/dtimepicker/jquery.datetimepicker.full.js', array( 'jquery' ), $this->plugin_version, true );
		wp_register_script( 'speed-dating-with-zoom-datable-js', ZVC_PLUGIN_VENDOR_ASSETS_URL . '/datatable/jquery.dataTables.min.js', array( 'jquery' ), $this->plugin_version, true );
		...
		//Plugin Scripts
		wp_enqueue_style( 'speed-dating-with-zoom', ZVC_PLUGIN_ADMIN_ASSETS_URL . '/css/style.min.css', false, $this->plugin_version );
		wp_register_script( 'speed-dating-with-zoom-js', ZVC_PLUGIN_ADMIN_ASSETS_URL . '/js/script.min.js', array(
			'jquery',
			'speed-dating-with-zoom-select2-js',
			'speed-dating-with-zoom-timepicker-js',
			'speed-dating-with-zoom-datable-js',
			'underscore'
		), $this->plugin_version, true );

		wp_localize_script( 'speed-dating-with-zoom-js', 'zvc_ajax', array(
			'ajaxurl'      => admin_url( 'admin-ajax.php' ),
			'zvc_security' => wp_create_nonce( "_nonce_zvc_security" ),
			'lang'         => array(
				'confirm_end'  => __( "Are you sure you want to end this meeting ? Users won't be able to join this meeting shown from the shortcode.", "speed-dating-with-zoom" ),
				'host_id_search'  => __( "Add a valid Host ID or Email address.", "speed-dating-with-zoom" )
			)
		) );
	}
	...
	public function load_plugin_textdomain() {
		load_plugin_textdomain( 'speed-dating-with-zoom', false, ZVC_PLUGIN_LANGUAGE_PATH );
	}
	...
	public function action_link( $actions, $plugin_file ) {
		...
		if ( $plugin == $plugin_file ) {
			$settings = array( 'settings' => '<a href="' . admin_url( 'edit.php?post_type=zoom-meetings&page=zoom-video-conferencing-settings' ) . '">' . __( 'Settings', 'speed-dating-with-zoom' ) . '</a>' );
