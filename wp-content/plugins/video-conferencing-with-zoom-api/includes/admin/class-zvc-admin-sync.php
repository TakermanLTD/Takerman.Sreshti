...
class Zoom_Video_Conferencing_Admin_Sync {
	...
	public static function render() {
		...
		if ( empty( $zoom_api_key ) || empty( $zoom_api_secret ) ) {
			echo '<p>' . __( 'API keys are not configured properly ! Please configure them before syncing.', 'speed-dating-with-zoom' ) . '</p>';

			return;
		}

		wp_enqueue_script( 'speed-dating-with-zoom-js' );
		wp_localize_script( 'speed-dating-with-zoom-js', 'vczapi_sync_i10n', array(
			'before_sync'              => __( "Fetching data.. Please wait this might take some time depending on how many Zoom Meetings you have.", "speed-dating-with-zoom" ),
			'total_records_found'      => __( "Total Records Found", "speed-dating-with-zoom" ),
			'total_not_synced_records' => __( "Total Not Synced Records", "speed-dating-with-zoom" ),
			'select2_placeholder'      => __( "Choose meeting from here to sync.", "speed-dating-with-zoom" ),
			'sync_btn'                 => __( "Sync Now", "speed-dating-with-zoom" ),
			'sync_start'               => __( 'Starting sync process.. Please wait for some time. Do not close this window until completed.', "speed-dating-with-zoom" ),
			'sync_error'               => __( 'Opps ! You have not selected any meeting to sync yet. Select one or two and this section will be filled by happiness for you !!', "speed-dating-with-zoom" ),
			'sync_completed'           => __( 'Meeting sync has been completed !', "speed-dating-with-zoom" ),
		) );
	...
	public function sync() {
		...
		$zoom_api_secret = get_option( 'zoom_api_secret' );
		if ( empty( $zoom_api_key ) || empty( $zoom_api_secret ) ) {
			wp_send_json_error( __( 'API keys are not configured properly ! Please configure them before syncing.', 'speed-dating-with-zoom' ) );
		}
		...
		if ( $type === "check" ) {
			...
			if ( ! empty( $meetings ) ) {
				...
				if ( ! empty( $meetings['meetings'] ) ) {
					...
					wp_send_json_error( __( "No Meetings Found !", 'speed-dating-with-zoom' ) );
				}
			} else {
				wp_send_json_error( __( "No Meetings Found !", 'speed-dating-with-zoom' ) );
			}
		...
		if ( $type === "sync" ) {
			...
			if ( ! empty( $meeting_id ) && ! in_array( $meeting_id, $db_meetings ) ) {
				...
				$data = array(
					'msg'        => __( "No meeting is selected or selected meeting already exists", 'speed-dating-with-zoom' ) . ': <strong>' . $meeting_id . '</strong>',
	...
	private function create_meeting( $meeting_obj ) {
		...
		if ( ! empty( $post_id ) ) {
			...
			$data = array(
				'msg'        => __( "Successfully imported meeting with ID", 'speed-dating-with-zoom' ) . ': <strong>' . $meeting_obj->id . '</strong>',
				'meeting_id' => $meeting_obj->id
			);
			wp_send_json_success( $data );
		} else {
			$data = array(
				'msg'        => __( "Failed to import meeting with ID", 'speed-dating-with-zoom' ) . ': <strong>' . $meeting_obj->id . '</strong>',
				'meeting_id' => $meeting_obj->id
			);
			wp_send_json_success( $data );
		}
