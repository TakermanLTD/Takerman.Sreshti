...
class Zoom_Video_Conferencing_Admin_PostType {
	...
	public function addFiltersOnSubSubSub( $views ) {
		...
		$views['upcoming'] = sprintf( '<a href="%s" ' . $upcoming . '>' . __( "Upcoming", "speed-dating-with-zoom" ) . '</a>', admin_url( '/edit.php?post_type=zoom-meetings&type=upcoming' ) );
		$views['past']     = sprintf( '<a href="%s" ' . $past . '>' . __( "Past", "speed-dating-with-zoom" ) . '</a>', admin_url( '/edit.php?post_type=zoom-meetings&type=past' ) );
	...
	public function add_columns( $columns ) {
		$columns['type']          = __( 'Type', 'speed-dating-with-zoom' );
		$columns['start_meeting'] = __( 'Start Meeting', 'speed-dating-with-zoom' );
		$columns['start_date']    = __( 'Start Date', 'speed-dating-with-zoom' );
		$columns['meeting_id']    = __( 'Meeting ID', 'speed-dating-with-zoom' );
		$columns['meeting_state'] = __( 'Meeting State', 'speed-dating-with-zoom' );
		...
		switch ( $column ) {
			case 'type':
				if ( ! empty( $meeting ) && ! empty( $meeting->type ) && ( $meeting->type === 5 || $meeting->type === 6 || $meeting->type === 9 ) ) {
					_e( 'Webinar', 'speed-dating-with-zoom' );
				} else {
					_e( 'Meeting', 'speed-dating-with-zoom' );
				}
				break;
			case 'start_meeting' :
				if ( ! empty( $meeting ) && ! empty( $meeting->start_url ) ) {
					...
					_e( 'Meeting not created yet.', 'speed-dating-with-zoom' );
				}
				break;
			case 'start_date' :
				if ( ! empty( $meeting ) && ! empty( $meeting->code ) && ! empty( $meeting->message ) ) {
					...
					_e( 'Recurring Meeting', 'speed-dating-with-zoom' );
				} else {
					_e( 'Meeting not created yet.', 'speed-dating-with-zoom' );
				}
				break;
			case 'meeting_id' :
				...
				break;
			case 'meeting_state' :
				wp_enqueue_script( 'speed-dating-with-zoom-js' );
				if ( ! empty( $meeting ) ) {
					if ( ! empty( $meeting->code ) && ! empty( $meeting->message ) ) {
						echo $meeting->message;
					} else if ( empty( $meeting->state ) ) { ?>
                        <a href="javascript:void(0);" class="vczapi-meeting-state-change" data-type="post_type" data-state="end" data-postid="<?php echo $post_id; ?>" data-id="<?php echo $meeting->id ?>"><?php _e( 'Disable Join', 'speed-dating-with-zoom' ); ?></a>
                        <p class="description"><?php _e( 'Restrict users to join this meeting before the start time or after the meeting is completed.', 'speed-dating-with-zoom' ); ?></p>
					<?php } else { ?>
                        <a href="javascript:void(0);" class="vczapi-meeting-state-change" data-type="post_type" data-state="resume" data-postid="<?php echo $post_id; ?>" data-id="<?php echo $meeting->id ?>"><?php _e( 'Enable Join', 'speed-dating-with-zoom' ); ?></a>
                        <p class="description"><?php _e( 'Resuming this will enable users to join this meeting.', 'speed-dating-with-zoom' ); ?></p>
					<?php }
				} else {
					_e( 'Meeting not created yet.', 'speed-dating-with-zoom' );
				}
	...
	public function register_taxonomy() {
		...
		$labels = array(
			'name' => _x( 'Category', 'Zoom Category Name', 'speed-dating-with-zoom' ),
		);
	...
	public function register_post_type() {
		$labels = apply_filters( 'vczapi_admin_cpt_labels', array(
			'name'               => _x( 'Zoom Meetings and Webinars', 'Zoom Meetings and Webinars', 'speed-dating-with-zoom' ),
			'singular_name'      => _x( 'Zoom Meeting', 'Zoom Meeting', 'speed-dating-with-zoom' ),
			'menu_name'          => _x( 'Zoom Meeting', 'Zoom Meeting', 'speed-dating-with-zoom' ),
			'name_admin_bar'     => _x( 'Zoom Meeting', 'Zoom Meeting', 'speed-dating-with-zoom' ),
			'add_new'            => __( 'Add New', 'speed-dating-with-zoom' ),
			'add_new_item'       => __( 'Add New meeting', 'speed-dating-with-zoom' ),
			'new_item'           => __( 'New meeting', 'speed-dating-with-zoom' ),
			'edit_item'          => __( 'Edit meeting', 'speed-dating-with-zoom' ),
			'view_item'          => __( 'View meetings', 'speed-dating-with-zoom' ),
			'all_items'          => __( 'All Meetings', 'speed-dating-with-zoom' ),
			'search_items'       => __( 'Search meetings', 'speed-dating-with-zoom' ),
			'parent_item_colon'  => __( 'Parent meetings:', 'speed-dating-with-zoom' ),
			'not_found'          => __( 'No meetings found.', 'speed-dating-with-zoom' ),
			'not_found_in_trash' => __( 'No meetings found in Trash.', 'speed-dating-with-zoom' ),
		) );
	...
	public function add_metabox() {
		add_meta_box( 'zoom-meeting-meta', __( 'Zoom Details', 'speed-dating-with-zoom' ), array(
			$this,
			'render_metabox'
		), $this->post_type, 'normal' );
		add_meta_box( 'zoom-meeting-meta-side', __( 'Meeting Details', 'speed-dating-with-zoom' ), array(
			$this,
			'rendor_sidebox'
		), $this->post_type, 'side', 'high' );
		add_meta_box( 'zoom-meeting-debug-meta', __( 'Debug Log', 'speed-dating-with-zoom' ), array(
			$this,
			'debug_metabox'
		), $this->post_type, 'normal' );
		if ( is_plugin_inactive( 'vczapi-woo-addon/vczapi-woo-addon.php' ) && is_plugin_inactive( 'vczapi-woocommerce-addon/vczapi-woocommerce-addon.php' ) ) {
			add_meta_box( 'zoom-meeting-woo-integration-info', __( 'WooCommerce Integration?', 'speed-dating-with-zoom' ), array(
				$this,
				'render_woo_sidebox'
	...
	public function render_metabox( $post ) {
		...
		wp_enqueue_script( 'speed-dating-with-zoom-js' );
		wp_enqueue_script( 'speed-dating-with-zoom-select2-js' );
		wp_enqueue_script( 'speed-dating-with-zoom-timepicker-js' );
        ...
        <div class="zoom-metabox-wrapper">
			...
			if ( ! empty( $meeting_details ) ) {
				if ( ! empty( $meeting_details->code ) && ! empty( $meeting_details->message ) ) {
					?>
                    <p>
                        <strong><?php _e( 'Meeting has not been created for this post yet. Publish your meeting or hit update to create a new one for this post !', 'speed-dating-with-zoom' ) ?></strong>
                    </p>
					<?php
                    ...
                    <div class="zoom-metabox-content">
                        <p><a target="_blank" href="<?php echo esc_url( $meeting_details->start_url ); ?>" title="Start URL"><?php _e( 'Start Meeting', 'speed-dating-with-zoom' ) ?></a></p>
                        <p><a target="_blank" href="<?php echo esc_url( $join_url ); ?>" title="Start URL"><?php _e( 'Join Meeting', 'speed-dating-with-zoom' ) ?></a></p>
                        <p><a target="_blank" href="<?php echo esc_url( $zoom_host_url ); ?>" title="Start URL"><?php _e( 'Start via Browser', 'speed-dating-with-zoom' ) ?></a></p>
                        <p><strong><?php _e( 'Meeting ID', 'speed-dating-with-zoom' ) ?>:</strong> <?php echo $meeting_details->id; ?></p>
			...
			<?php } ?>
            <div class="zoom-metabox-content">
                <p><?php _e( 'Requires Login?', 'speed-dating-with-zoom' ); ?>
                ...
                <p class="description"><?php _e( 'Only logged in users of this site will be able to join this meeting.', 'speed-dating-with-zoom' ); ?></p>
                <p><?php _e( 'Hide Join via browser link ?', 'speed-dating-with-zoom' ); ?>
                ...
                <p class="description"><?php _e( 'This will disable join via browser link in frontend page.', 'speed-dating-with-zoom' ); ?></p>
            </div>
        </div>
		<?php
	}
	...
	public function debug_metabox( $post ) {
        ...
        <div class="zoom-metabox-wrapper">
            <div class="zoom-metabox-content">
                <p><?php _e( 'Enable Debug?', 'speed-dating-with-zoom' ); ?>
