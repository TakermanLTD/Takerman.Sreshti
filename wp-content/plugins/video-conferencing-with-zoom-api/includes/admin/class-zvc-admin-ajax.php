...
class Zoom_Video_Conferencing_Admin_Ajax {
	...
	public function delete_meeting() {
		...
		if ( $meeting_id ) {
			...
			wp_send_json( array( 'error' => 0, 'msg' => __( "Deleted Meeting with ID", "speed-dating-with-zoom" ) . ': ' . $meeting_id ) );
		} else {
			wp_send_json( array(
				'error' => 1,
				'msg'   => __( "An error occured. Host ID and Meeting ID not defined properly.", "speed-dating-with-zoom" )
			) );
		}
	...
	public function delete_bulk_meeting() {
		...
		if ( ! empty( $meeting_ids ) ) {
			...
			if ( $deleted ) {
				wp_send_json( array(
					'error' => 0,
					'msg'   => sprintf( __( "Deleted %d Meeting(s).", "speed-dating-with-zoom" ), $meeting_count )
				) );
			}
		} else {
		...
		if ( ! empty( $users ) ) {
			foreach ( $users as $user ) {
				...
				if ( ! empty( $zoom_users ) ) {
					...
					$host_id_field .= '<option value="">' . __( 'Not a Host', 'speed-dating-with-zoom' ) . '</option>';
