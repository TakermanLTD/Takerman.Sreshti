...
class Zoom_Video_Conferencing_Admin_Webinars {
	...
	public static function list_webinars() {
		...
		wp_enqueue_script( 'speed-dating-with-zoom-datable-js' );
	...
	private static function update_webinar() {
		...
		if ( ! empty( $meeting_updated->code ) ) {
			...
			self::set_message( 'updated', __( "Updated Webinar.", "speed-dating-with-zoom" ) );
		}
	...
	private static function create_webinar() {
		...
		if ( ! empty( $webinar_created->code ) ) {
			...
			self::set_message( 'updated', sprintf( __( "Created webinar %s at %s. Join %s", "speed-dating-with-zoom" ), $webinar_created->topic, $webinar_created->created_at, "<a target='_blank' href='" . $webinar_created->join_url . "'>Here</a>" ) );
