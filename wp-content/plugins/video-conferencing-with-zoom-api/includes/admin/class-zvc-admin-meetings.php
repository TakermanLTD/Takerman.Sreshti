...
class Zoom_Video_Conferencing_Admin_Meetings {
	...
	private static function update_meeting() {
		...
		if ( ! empty( $meeting_updated->code ) ) {
			...
			self::set_message( 'updated', __( "Updated meeting.", "speed-dating-with-zoom" ) );
		}
	...
	private static function create_meeting() {
		...
		if ( ! empty( $meeting_created->code ) ) {
			...
			self::set_message( 'updated', sprintf( __( "Created meeting %s at %s. Join %s", "speed-dating-with-zoom" ), $meeting_created->topic, $meeting_created->created_at, "<a target='_blank' href='" . $meeting_created->join_url . "'>Here</a>" ) );
