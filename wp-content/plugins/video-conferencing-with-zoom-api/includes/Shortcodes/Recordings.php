...
class Recordings {
					...
					if ( ! empty( $recording->recording_files ) ) {
						foreach ( $recording->recording_files as $files ) {
                            ...
                            <ul class="vczapi-modal-list vczapi-modal-list-<?php echo $files->id; ?>">
                                <li><strong><?php _e( 'File Type', 'speed-dating-with-zoom' ); ?>: </strong> <?php echo $files->file_type; ?></li>
                                <li><strong><?php _e( 'File Size', 'speed-dating-with-zoom' ); ?>: </strong> <?php echo vczapi_filesize_converter( $files->file_size ); ?></li>
								<?php
								if ( true == apply_filters( 'vczapi_recordings_show_password', false ) && isset( $recording->password ) && ! empty( $recording->password ) ) {
									?>
                                    <li><strong><?php _e( 'Password:', 'speed-dating-with-zoom' ); ?></strong> <?php echo $recording->password; ?></li>
								<?php }
								?>
                                <li><strong><?php _e( 'Play', 'speed-dating-with-zoom' ); ?>: </strong><a href="<?php echo $files->play_url; ?>" target="_blank"><?php _e( 'Play', 'speed-dating-with-zoom' ); ?></a></li>
	...
	public function recordings_by_user( $atts ) {
		...
		if ( empty( $atts['host_id'] ) ) {
			echo '<h3 class="no-host-id-defined"><strong style="color:red;">' . __( 'Invalid HOST ID. Please define a host ID to show recordings based on host.', 'speed-dating-with-zoom' ) . '</h3>';
		...
		wp_localize_script( 'speed-dating-with-zoom-shortcode-js', 'vczapi_recordings_data', array(
			'downloadable' => $downloadable,
			'loading'      => __( 'Loading recordings.. Please wait..', 'speed-dating-with-zoom' )
		) );
		...
		if ( ! empty( $recordings ) ) {
			...
			_e( "No recordings found.", "speed-dating-with-zoom" );
		}
	...
	public function recordings_by_meeting_id( $atts ) {
		...
		if ( empty( $atts['meeting_id'] ) ) {
			echo '<h3 class="no-meeting-id-defined"><strong style="color:red;">' . __( 'Invalid Meeting ID.', 'speed-dating-with-zoom' ) . '</h3>';
