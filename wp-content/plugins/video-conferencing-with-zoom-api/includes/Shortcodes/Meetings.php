...
class Meetings {
		...
		if ( ! empty( $meeting ) && ! empty( $meeting->code ) ) {
			...
			if ( ! empty( $link_only ) && $link_only === "yes" ) {
				...
				if ( $meeting ) {
					...
					printf( __( 'Please try again ! Some error occured while trying to fetch meeting with id:  %d', 'speed-dating-with-zoom' ), $meeting_id );
				}
			}
		}
	...
	public function show_meeting_by_postTypeID( $atts ) {
		...
		if ( empty( $post_id ) ) {
			echo '<h4 class="no-meeting-id"><strong style="color:red;">' . __( 'ERROR: ', 'speed-dating-with-zoom' ) . '</strong>' . __( 'No post id set in the shortcode', 'speed-dating-with-zoom' ) . '</h4>';
		...
		if ( $meeting->have_posts() ) {
			...
			echo "<p>" . __( 'This post does not exist.', 'speed-dating-with-zoom' ) . "</p>";
		}
	...
	public function list_meeting_ajax_handler() {
		...
		if ( $zoom_meetings->have_posts() ) {
			...
			echo "<p class='vczapi-no-meeting-found'>" . __( 'No Meetings found.', 'speed-dating-with-zoom' ) . "</p>";
		}
	...
	public function list_live_host_meetings( $atts ) {
		...
		if ( empty( $meetings ) || $cache_expiration < time() ) {
			...
			if ( isset( $decoded_meetings->meetings ) ) {
				...
				return __( 'Could not retrieve meetings, check Host ID', 'speed-dating-with-zoom' );
			}
		}
