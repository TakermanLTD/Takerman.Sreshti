...
function vczapi_check_connection_error() {
    ...
    <div id="message" class="notice notice-warning is-dismissible">
        <p>
			<?php
			esc_html_e( 'Please check your internet connection or API keys. Zoom API is not able to connect with Zoom servers at the moment.', 'speed-dating-with-zoom' );
			?>
        </p>
    </div>
	<?php
...
function video_conferencing_zoom_api_show_like_popup() {
	...
	if ( ! get_transient( '_vczapi_dismiss_notice' ) ) {
		?>
        <div id="message" class="notice notice-warning is-dismissible">
            <h3><?php esc_html_e( 'Like this plugin ?', 'speed-dating-with-zoom' ); ?></h3>
            <p>
				<?php
				printf( esc_html__( 'Please consider giving a %s if you found this useful at wordpress.org or ', 'speed-dating-with-zoom' ), '<a href="https://wordpress.org/support/plugin/speed-dating-with-zoom/reviews/#new-post">5 star thumbs up</a>' );
				printf( esc_html__( 'check %s for shortcode references.', 'speed-dating-with-zoom' ), '<a href="' . admin_url( 'edit.php?post_type=zoom-meetings&page=zoom-video-conferencing-settings' ) . '">settings</a>.' );
				?>
...
function video_conferencing_zoom_api_show_api_notice() {
	...
	if ( empty( $notice ) ) {
        ...
        <div id="message" class="notice notice-success"><p style="font-size:16px;">
                <strong><?php _e( "Do not get confused here !!", "speed-dating-with-zoom" ); ?></strong>
            <p>
                <strong><?php _e( "Please read !!! These below meetings are directly from your zoom.us account via API connection. Meetings added from here won't show up on your Post Type list. This will only create meeting in your zoom.us account !", "speed-dating-with-zoom" ); ?></strong>
                <a href="javascript:void(0);" class="zvc-dismiss-message"><?php _e( "I understand ! Don't show this again !", "speed-dating-with-zoom" ); ?></a>
            </p></div>
		<?php
	}
}
...
function vczapi_get_browser_join_links( $post_id, $meeting_id, $password = false, $seperator = false ) {
	...
	if ( ! vczapi_check_disable_joinViaBrowser() ) {
		if ( ! empty( $password ) && empty( $embed_password_join_link ) ) {
			...
			return $seperator . '<a target="_blank" rel="nofollow" href="' . esc_url( $query ) . '" class="btn btn-join-link btn-join-via-browser">' . apply_filters( 'vczapi_join_meeting_via_browser_text', __( 'Влез чрез браузър', 'speed-dating-with-zoom' ) ) . '</a>';
		} else {
...
function vczapi_get_browser_join_shortcode( $meeting_id, $password = false, $link_only = false, $seperator = false ) {
	...
	if ( ! vczapi_check_disable_joinViaBrowser() ) {
		if ( ! empty( $password ) && empty( $embed_password_join_link ) ) {
			$encrypt_pwd = vczapi_encrypt_decrypt( 'encrypt', $password );
			$query       = add_query_arg( array( 'pak' => $encrypt_pwd, 'join' => $encrypt_meeting_id, 'type' => 'meeting' ), $link );
			$result      = $seperator . '<a target="_blank" rel="nofollow" href="' . esc_url( $query ) . '" class="btn btn-join-link btn-join-via-browser">' . apply_filters( 'vczapi_join_meeting_via_browser_text', __( 'Влез чрез браузър', 'speed-dating-with-zoom' ) ) . '</a>';
			$link        = esc_url( $query );
		} else {
			$query  = add_query_arg( array( 'join' => $encrypt_meeting_id, 'type' => 'meeting' ), $link );
			$result = $seperator . '<a target="_blank" rel="nofollow" href="' . esc_url( $query ) . '" class="btn btn-join-link btn-join-via-browser">' . apply_filters( 'vczapi_join_meeting_via_browser_text', __( 'Влез чрез браузър', 'speed-dating-with-zoom' ) ) . '</a>';
...
function vczapi_zoom_api_paginator( $response, $type = '' ) {
	...
	if ( ! empty( $response ) && $response->next_page_token ) {
		...
		?>
        <a href="<?php echo $next_page; ?>"><?php _e( 'Next Results', 'speed-dating-with-zoom' ); ?></a>
		<?php
	}
}
